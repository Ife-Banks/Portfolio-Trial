   ...
   useEffect(() => {
    ...
    const ContactItem = ({ label, value }) => (
        <li className='flex flex-col'>
            <h1 className='font-workSan text-lg font-medium'>{label}</h1>
            <span className='font-IBM italic text-sm -mt-1 font-light'>{value}</span>
        </li>
    );
  return (
    <div className="w-[90%] lg:w-[60%] border border-[#474C54] mx-auto lg:-mb-40 p-5 flex flex-col gap-5 rounded-2xl relative bg-black/20 backdrop-blur-sm shadow-[0_8px_32px_rgba(255,255,255,0.2)]"
    id='contact'>
       <div 
        ref={bg1Ref}
        ...
        <main className='flex flex-col xl:flex-row items-center justify-evenly'>
            <Headings
                text="Get In Touch"
                subText="Open to new opportunities, collaborations, and meaningful conversations with forward-thinking individuals and teams. To view the portfolio deck or discuss a potential project, feel free to reach out via email or book a call."
                styh1="text-left"
                styp="w-[100%] text-sm"
            />

            <ul className='flex flex-col gap-5 p-2 text-white w-full xl:w-fit place-self-end'>
                <ContactItem
                    label="Email:"
